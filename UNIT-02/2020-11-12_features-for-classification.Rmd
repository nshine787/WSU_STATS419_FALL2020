...
Some of the numbers have the same "curved" parts. The numbers zero, two, three, six, and eight are all drawn with curves. I hypothesize that these numbers will be mistaken for each other because of this reason. The numbers one, seven, and four all are constructed with mostly straight lines, and they do not overlap as much. I'm not sure what to say for five. I think five will be a more unique case.  
...
We could make our classifier have a smaller penalty for mis-classifying a "0," this way it does not over correct when it classifies the numbers.
...
sub = wildcardSearch( paste0("24x24",".*"), "data.id", df=final$data);
## data.form
sub = wildcardSearch( paste0("*.","b",".*"), "data.id", df=sub);
## matrix.division 
sub = wildcardSearch( paste0("*.",6,"_*"), "data.id", df=sub);
## matrix.features            
sub = wildcardSearch( paste0("*_","rowsums"), "data.id", df=sub);
## comparison.methods
sub = subsetDataFrame( sub, "comparison.method", "==" , "sim.cosine");
## decision.stats
result = list();
...
for(decision.stats in option$decision.stats)
    ...
    #test1 = sum(sub$true == sub$best) / nrow(sub); 
    test1 = sum(stats1[,1] == stats1[,3]) / nrow(stats1); # compare to individual test...
    if(key.name == "stats.top^1") { pick.one=stats1; }
    result[[key.name]] = test1;
}
result;
pick.one;
```
I've got 0.6 with a 24x24 binary matrix with 6 cuts using row sums and cosine similarity with stats.top^1. This has the twos correctly classified so they are no longer bullied by the zeroes. However, because of this the sevens are now bullied by the twos instead of the zeroes.
